...
export function generateReportCardPDF(data: ReportCardData, classRank?: number, classTotal?: number): void {
  ...
  // ── Page 2: visual performance (bars + grade mix) ───────────────────────────
  const chartRows = data.marks.map((m) => ({
    subject: m.subject_name || m.subject_code || 'Subject',
    obtained: parseFloat(String(m.marks_obtained)) || 0,
    total: parseFloat(String(m.total_marks)) || 0,
    grade: String(m.grade || 'F8'),
  }));
  drawReportCardPerformancePage(doc, {
    studentLabel: `${data.student.first_name} ${data.student.last_name}`,
    termYear: [data.summary.term, data.summary.academicYear].filter(Boolean).join(' · '),
    rows: chartRows,
  });
